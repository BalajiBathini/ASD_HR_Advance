...
class Dashboard(models.Model):
    ...
    parent_menu_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref(
            "asd_cust_dashboard.parent_dashboard_menu"
        ),
        domain=[("action", "=", False)],
        tracking=True,
    )
            ...
            cron_id = self.env["ir.cron"].create(
                {
                    ...
                    "model_id": self.env.ref(
                        "asd_cust_dashboard.model_dashboard_dashboard"
                    ).id,
                    "state": "code",
    ...
    def action_view_charts(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "asd_cust_dashboard.dashboard_chart_action"
        )
        if len(chart_ids) > 1:
            ...
            form_view = [
                (
                    self.env.ref("asd_cust_dashboard.dashboard_chart_form_view").id,
                    "form",
                )
            ]
    ...
    def get_charts_details(self):
        ...
        if (
            user
            and user.has_group("asd_cust_dashboard.group_dashboard_user")
            and not user.has_group("asd_cust_dashboard.group_dashboard_manager")
        ):
...
class DashboardMail(models.Model):
    ...
    def _default_mail_template(self):
        ...
        """
        if self.env.ref("asd_cust_dashboard.mail_template_dashboard_chart"):
            return self.env.ref(
                "asd_cust_dashboard.mail_template_dashboard_chart"
            ).id
